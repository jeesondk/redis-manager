...
public interface ConnectionResponseMapper {
    ...
    default String toModeString(RedisConnectionType t) {
    ...
    default RedisServerInfo toServerInfo(ConnectionConfig cfg) {
        ...
        if (!endpoints.isEmpty()) {
            RedisHostAndPort first = parseHostPort(endpoints.get(0));
        ...
        return new RedisServerInfo(hosts, port, cfg.type, defaultDb, timeout);
    }

    /** Parses formats like \"host:port\"; strips scheme and credentials if present. */
    private static RedisHostAndPort parseHostPort(String s) {
        if (s == null) return new RedisHostAndPort("", 0);
        String t = s.trim();
        ...
        if (colon > 0 && colon < t.length() - 1) {
            ...
            try {
                ...
                return new RedisHostAndPort(host, p);
            } catch (NumberFormatException ignored) { }
        }
        return new RedisHostAndPort(t, 0);
    }
}
